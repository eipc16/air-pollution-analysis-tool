...
@RequestMapping("/groundMeasurements")
@Controller
public class GroundMeasurementsController {
    ...
    public void getFile(@PathVariable("orderId") String orderId, HttpServletResponse response) {
        ...
        try {
            ...
            Page<GroundMeasurement> measurementPage = null;
            int currentPage = 0;

            os.write(headerString.getBytes(StandardCharsets.UTF_8));

            do {
                measurementPage = groundMeasurementsService.findByOrderId(orderId, currentPage, BATCH_SIZE);
                currentPage++;

                StringBuilder stringBuilder = new StringBuilder();

                for (GroundMeasurement measurement : measurementPage) {
